...
<div class="page-title">
  <div class="container">
    <div class="row">
      <div class="col-md-12">
        <h3> {{ __('Edit Page') }}</h3>
       {{ Breadcrumbs::render('editPage',$page->id) }}
      </div>
    </div>
  </div>
</div>
  ...
  @endif   
    ...
    <div class="col-md-12">
      <div class="panel panel-white">
        ...
        <form enctype="multipart/form-data" action="{{ route('admin.editpageupdate', $page->id) }}" class="form-horizontal" method="post">
            @csrf
            <div class="form-group">
              <label for="post_title" class="col-sm-2 control-label required">Post Title</label>
              <div class="col-sm-10">
                <input type="text" placeholder="Post Title" value="{{ $page->post_title }}" class="form-control" name="post_title" />
         ...
         <div class="form-group">
          ...
          <div class="col-sm-10">
            <input type="text" name="meta_title" id="meta_title" value="{{ $page->meta_title }}" autocomplete="off" placeholder="Meta Title" class="form-control">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <textarea name="meta_description" id="meta_description" autocomplete="off" placeholder="Meta Description" class="form-control" rows="2" cols="5">{{ $page->meta_description }}</textarea>
          </div>
        </div>
        <div class="form-group">
          <label for="meta_keyword" class="col-sm-2 control-label optional">Meta Keyword</label>
          <div class="col-sm-10">
            <textarea name="meta_keyword" id="meta_keyword" autocomplete="off" placeholder="Meta Keyword" class="form-control" rows="2" cols="5">{{ $page->meta_keyword }}</textarea>
          </div>
        </div>
      <div class="form-group">
          ...
          <div class="col-sm-10">
            <select name="parent_post" id="parent_post" class="form-control">
                ...
                <option value="{{ $parentPage->id }}" {{ ($parentPage->id==$page->parent_post_id)?'selected':'' }}>{{ $parentPage->post_title }}</option>    
                @endforeach
            </select>
          </div>
        </div> 
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <select name="layout_slug" id="layout_slug" class="form-control">
                ...
                <option value="{{ $layout->slug }}" {{ ($layout->slug==$page->layout_slug)?'selected':'' }}>{{ $layout->name }}</option>    
                @endforeach
            </select>
          </div>
        </div>
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <textarea name="post_content" id="post_content" autocomplete="off" placeholder="This is your first post. Edit or delete it, then start writing!" class="form-control summernote" rows="5" cols="5">{{ $page->post_content }}</textarea>
          </div>
        </div>
        <div class="form-group">
          <label for="post_excerpt" class="col-sm-2 control-label optional">Post Excerpt</label>
          <div class="col-sm-10">
            <textarea name="post_excerpt" id="post_excerpt" autocomplete="off" class="form-control" rows="5" cols="5">{{ $page->post_excerpt }}</textarea>
          </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <input type="text" name="sort_order" id="sort_order" value="{{ $page->sort_order }}" autocomplete="off" placeholder="Sort Order" class="form-control">
          </div>
        </div>
                    ...
                    <tbody class="section{{ $sn }}">
                        <tr>
                            ...
                            @if ($customOption->image)
                                <img src="{{ config('AWS_URL','http://gds-image.s3-ap-south-1.amazonaws.com') }}/pages/custom-images/{{ $customOption->image }}" width="50px" />
                            @endif
                        ...
                        @foreach ($customOption->customFieldOptions as $customSubOption)
                            ...
                            @if ($customSubOption->custom_image)
                                <img src="{{ config('AWS_URL','http://gds-image.s3-ap-south-1.amazonaws.com') }}/pages/custom-images/custom-sub-images/{{ $customSubOption->custom_image }}" width="50px" />
                            @endif
